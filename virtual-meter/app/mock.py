...
def _rpc_response_from_body(
    body: dict[str, Any], result: dict[str, Any] | None, error: dict[str, Any] | None
) -> web.Response:
    response: dict[str, Any] = {
        "jsonrpc": "2.0",
        "id": body.get("id", 1),
...
def _result_only_response(
    result: dict[str, Any] | None, error: dict[str, Any] | None = None
) -> web.Response:
    if error is not None:
...
def _build_components(
    status_map: dict[str, Any], include: set[str] | None = None
) -> list[dict[str, Any]]:
...
def _mock_methods(
    settings: Settings,
) -> dict[str, Callable[[web.Request, dict[str, Any] | None], dict[str, Any]]]:
    return {
        "Shelly.GetStatus": lambda r, _p: {
            **_build_status(r),
            "em:0": _mock_em_status(settings),
        },
        "Shelly.GetConfig": lambda _r, _p: MOCK_SHELLY_CONFIG,
        "Shelly.ListMethods": lambda _r, _p: {
            "methods": sorted(_mock_methods(settings).keys())
        },
        ...
        "Shelly.GetComponents": lambda r, params: {
            ...
            "components": _build_components(
                ...
                set((params or {}).get("include", []))
                if (params or {}).get("include")
                else None,
            ),
        },
        ...
        "EM1Data.GetNetEnergies": lambda _r, _p: {
            "total_act": 0.0,
            "total_act_ret": 0.0,
        },
...
def create_app(settings: Settings) -> web.Application:
    ...
    async def shelly_get_status(request: web.Request) -> web.Response:
        ...
        return web.json_response(
            {**_build_status(request), "em:0": _mock_em_status(settings)}
        )
